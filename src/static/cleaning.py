...
def clean_data(sim_path,realization):
    # load the file
    filepath = os.path.join(sim_path,f'trj{realization}.csv')
    ...
    if is_low_memory(df):
        ...
        dfclean.to_csv(os.path.join(sim_path,f'xtrj{realization}.csv'))
    elif is_regular(df):
...
DRIVE = f'/home/frieren/BIG/reentrancy/{usr_input}/'
SIZES = next(os.walk(DRIVE))[1]

for size in SIZES:
    ctrjpath = os.path.join(DRIVE,size)
    for r in range(1,11):
        trypath = os.path.join(ctrjpath,f'xtrj{r}.csv')
